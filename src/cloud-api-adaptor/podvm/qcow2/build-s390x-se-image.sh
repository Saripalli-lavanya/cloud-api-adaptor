...
boot_uuid1=$(sudo blkid ${tmp_nbd}1 -s PARTUUID -o value)
export boot_uuid1
...
LUKS_NAME="cryptroot"
echo YES | sudo cryptsetup open ${tmp_nbd}2 $LUKS_NAME --key-file ${workdir}/rootkeys/rootkey.bin

# Format and copy root filesystem
echo "Copying root filesystem to encrypted partition"
sudo mke2fs -t ext4 -L root /dev/mapper/$LUKS_NAME
...
sudo bash -c 'cat <<END > /etc/fstab
# This file was auto-generated
PARTUUID=${boot_uuid1}    /        ext4  defaults 1 1
/dev/mapper/${LUKS_NAME} /        ext4  defaults 0 0
END'
...
sudo rm -rf ${src_mnt} ${dst_mnt} || true

# Close encrypted root partition
